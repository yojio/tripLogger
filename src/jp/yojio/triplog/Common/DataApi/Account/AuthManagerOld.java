...
public class AuthManagerOld implements AuthManager {
  ...
  /**
   * The number of handlers at the beginning of the above list that shouldn't be
   * removed after they are called.
   */
  private int stickyNewTokenListenerCount;

  /**
   * AuthManager requires many of the same parameters as
   * {@link GoogleLoginServiceHelper#getCredentials(Activity, int, Bundle, boolean, String, boolean)}
   ...
   * @param extras
   *          A {@link Bundle} of extras for {@link GoogleLoginServiceHelper}
   * @param requireGoogle
   *          True if the account must be a Google account
   * @param service
   *          The name of the service to authenticate as
   */
  public AuthManagerOld(Activity activity, int code, Bundle extras, boolean requireGoogle, String service) {
   ...
   *
   * @see
   * com.google.android.apps.mytracks.io.AuthManager#doLogin(java.lang.Runnable)
   */
  ...
  private class LoginRunnable implements Runnable {
    public void run() {
      GoogleLoginServiceHelper.getCredentials(activity, code, extras, requireGoogle, service, true);
    }
  }
   ...
   * (non-Javadoc)
   *
   * @see com.google.android.apps.mytracks.io.AuthManager#authResult(int,
  ...
  public boolean authResult(int resultCode, Intent results) {
    if (resultCode == Activity.RESULT_OK) {
      ...
      if (authToken == null) {
        GoogleLoginServiceHelper.getCredentials(activity, code, extras, requireGoogle, service, false);
        return true;
      } else {
   ...
   *
   * @see
   * com.google.android.apps.mytracks.io.AuthManager#invalidateAndRefresh(java
   * .lang.Runnable)
   */
   ...
   *
   * @param listener
   *          The {@link Runnable} to execute every time a new auth token is
   *          fetched
   */
   ...
   * {@link #addStickyNewTokenListener(Runnable)} above. If the {@link Runnable}
   * was added more than once, only the first occurrence will be removed.
   *
   * @param listener
   *          The {@link Runnable} to stop executing every time a new auth token
   *          is fetched
   */
  public void removeStickyNewTokenListener(Runnable listener) {
    synchronized (newTokenListeners) {
      if (stickyNewTokenListenerCount > 0 && newTokenListeners.remove(listener)) {
        stickyNewTokenListenerCount--;
      }
    }
  }
